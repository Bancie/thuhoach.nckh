...
\begin{proof}
	...
	Giả sử ta cần sắp xếp lại vị trí hai công việc này bằng cách tráo đổi vị trí với nhau thì ta nhận được chuỗi công việc $S'$, từ đó phương án là $k - j$. \eqref{jk_edd2}
	...
	Từ \eqref{LjS'}, \eqref{LkS'} và \eqref{dj<dk} ta dễ dàng nhận thấy
	...
	Từ \eqref{L_jS'>J_kS'}, ta có thể viết lại bất đẳng thức \eqref{edd_dk_max} như sau
...
\begin{proof}
	...
	Giả sử ta cần sắp xếp lại vị trí hai công việc này bằng cách tráo đổi vị trí với nhau thì ta nhận được chuỗi công việc $S'$, từ đó phương án là $k - j$. \eqref{jk_erd2}
	...
	Từ \eqref{CkS'_erd}, \eqref{CjS'_erd} và \eqref{rj<rk} ta dễ dàng nhận thấy
	...
	Từ \eqref{CjS'>CkS'_erd}, ta có thể viết lại bất đẳng thức \eqref{max<max_erd} như sau
	...
	Ta đặt chuỗi các công việc phía trước hai công việc $j$ và $k$ là $A$, từ đó ta có thể viết lại hệ bất đẳng thức \eqref{hbdt_erd} thành
	...
	Xét bất phương trình đầu tiên trong hệ \eqref{hbpt_erd_2}

	\begin{equation}
		\begin{array}{c c c c}
		& \max (C_A, r_j) + p_j &<& \max \left[ \max (C_A, r_k) + p_k, r_j \right] + p_j \\
		\Leftrightarrow & \max (C_A, r_j) &<& \max \left[ \max (C_A, r_k) + p_k, r_j \right]
		\end{array}
	\end{equation}
	...
	$C_A < \max \left[ \max (C_A, r_k) + p_k, r_j \right]$ hiển nhiên.
	
	\begin{equation}
		\begin{array}{c c c c}
			& r_j &<& \max \left[ \max (C_A, r_k) + p_k, r_j \right] \\
			\Leftrightarrow & r_j &<& \max (C_A, r_k) + p_k
		\end{array}
	\end{equation}
	...
	\begin{equation}
		\begin{array}{c c c c}
			& \max (C_A, r_j) + p_j &<& \max (C_A, r_k) + p_j \\
			\Leftrightarrow & \max (C_A, r_j) &<& \max (C_A, r_k) \\
			\Leftrightarrow & r_j &<& r_k \\
		\end{array}
	\end{equation}
